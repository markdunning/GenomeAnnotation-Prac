...
filters = "flybase_gene_id", values = rownames(topHits), mart = ensembl)
...
A full list of annotation packages is available on the Bioconductor \href{http://tinyurl.com/l7hsus5}{{\color{blue}website}}.
...
\subsubsection{Alternative annotation of RNA-seq results}
...
myInfo2 <- select(dm, keytype = "FLYBASE",
      columns =c("ENSEMBL", "CHRLOC","CHRLOCEND", "SYMBOL","GO") ,keys=rownames(topHits))

@


\subsection{Alternative counting of the pasilla dataset}
